  ...
  return (
    <div style={{ padding: '2.5rem', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        {/* Observations */}
        <div className="fade-in-delay-2" style={{ background: 'var(--card-bg)', borderRadius: '12px', padding: '1.5rem', border: '1px solid var(--border-light)' }}>
          <h2 style={{ fontSize: '1.1rem', marginBottom: '1.25rem' }}>Observation Notes</h2>
          {project.observations.length === 0 ? (
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>No notes yet. Add observations about aroma, taste, and appearance.</p>
          ) : (
        ...
        {/* Measurements log */}
        <div className="fade-in-delay-3" style={{ background: 'var(--card-bg)', borderRadius: '12px', padding: '1.5rem', border: '1px solid var(--border-light)' }}>
          <h2 style={{ fontSize: '1.1rem', marginBottom: '1.25rem' }}>Measurement Log</h2>
          {project.measurements.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '280px', overflowY: 'auto' }}>
              {project.measurements.slice().reverse().map(m => (
                <div key={m.id} style={{ display: 'grid', gridTemplateColumns: '80px repeat(4, 1fr)', gap: '0.5rem', padding: '0.625rem', background: 'var(--warm-white)', borderRadius: '6px', fontSize: '0.8rem' }}>
                  <span style={{ color: 'var(--text-muted)' }}>{format(parseISO(m.logged_at), 'MMM d')}</span>
